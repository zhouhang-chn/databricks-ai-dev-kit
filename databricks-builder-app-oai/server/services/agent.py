...
def _preview_value(value: object, max_chars: int = 1000) -> str:
...
def _event_preview(event: dict) -> str:
  ...
  if event_type in {'text', 'text_delta'}:
    return f'text={_preview_value(event.get("text", ""), 2000)}'
  if event_type == 'tool_use':
    return (
      f'tool_name={event.get("tool_name")} '
      f'tool_id={event.get("tool_id")} '
      f'input={_preview_value(event.get("tool_input", {}), 2000)}'
    )
  if event_type == 'tool_result':
    return (
      f'tool_use_id={event.get("tool_use_id")} '
      f'is_error={event.get("is_error")} '
      f'content={_preview_value(event.get("content", ""), 2000)}'
    )
  if event_type == 'system':
    return (
      f'subtype={event.get("subtype")} '
      f'data={_preview_value(event.get("data", {}), 2000)}'
    )
  ...
  if event_type == 'error':
    return f'error={_preview_value(event.get("error", ""), 2000)}'
  return _preview_value(event, 2000)
...
def _log_agent_event(event_number: int, event: dict) -> None:
  ...
  if event_type in {'text', 'text_delta'} and _is_error_like_text(str(event.get('text', ''))):
    logger.error('[AGENT] Event #%s: %s %s', event_number, event_type, preview)
  elif event_type == 'error' or event.get('is_error') is True:
    logger.error('[AGENT] Event #%s: %s %s', event_number, event_type, preview)
  else:
    logger.info('[AGENT] Event #%s: %s %s', event_number, event_type, preview)
...
async def stream_agent_response(
  ...
  ensure_logger_active(logger, set_propagate_false=True)
  logger.info(
    'Starting OpenAI agent runtime for project=%s session=%s message_len=%s',
    project_id,
    session_id or '<new>',
    len(message),
  )
