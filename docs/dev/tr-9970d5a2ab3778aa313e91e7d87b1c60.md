...
| Conversation History | 327 | ~93 | 0.1% |
| Tool Results | 3,875 | ~1,107 | 0.9% |
| Current Prompt | 36 | ~10 | 0.0% |
| **Truncated Data** (Missing from Trace) | - | ~128,811 | 99.1% |

## 2. Why is the token count so high?
The trace shows **130,022 input tokens**, but only **4,240 characters** were captured in the MLflow log.

### Key Drivers:
### Key Drivers:
1. **Invisible Baseline Context (~40,000+ tokens):** The MLflow tracer (`mlflow.anthropic.autolog`) currently **omits** the `system` prompt and `tools` definitions from the logged attributes, even though they are sent to the model. These account for the bulk of the 130,022 tokens.
...
- [ ] **Implement Tool Result Summarization:** (High Priority) Modify `get_table_stats_and_schema` to return only basic metadata by default.
